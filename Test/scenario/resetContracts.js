...
module.exports = callback => {
	...
	const projectContract_abi = JSON.parse(fs.readFileSync(path.resolve(__dirname, '..', '..', 'Contract', 'build', 'projectContract.abi')))
	const projectContract_bytecode = '0x' + fs.readFileSync(path.resolve(__dirname, '..', '..', 'Contract', 'build', 'projectContract.bin')).toString()

	const companyContract_abi = JSON.parse(fs.readFileSync(path.resolve(__dirname, '..', '..', 'Contract', 'build', 'company.abi')))
	const companyContract_bytecode = '0x' + fs.readFileSync(path.resolve(__dirname, '..', '..', 'Contract', 'build', 'company.bin')).toString()
